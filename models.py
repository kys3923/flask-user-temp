...
# [DB_TYPE]+[DB_CONNECTOR]://[USERNAME]:[PASSWORD]@[HOST]:[POST]/[DB_NAME]
db = SQLAlchemy(app)
...
class User(db.Model):
  ...
  bio = db.Column(db.String(150)) #max 150 characters
...
class Post(db.Model):
  ...
  author_id = db.Column(db.Integer, db.ForeignKey("users.id", ondelete="SET NULL")) #references postgres table

  author = db.relationship("User", back_populates="posts", lazy="subquery") # lazy="joined" will query User table at the same time, lazy=False
